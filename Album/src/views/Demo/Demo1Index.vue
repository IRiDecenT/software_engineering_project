...
import { getUserInfo, getAlbums } from '@/api/user'
...
async function getData() {
  ...
  const albumsResponse = await getAlbums(uid)
  console.log(userInfoResponse)
  console.log(albumsResponse)
  userInfo.value = await userInfoResponse
  albums.value = await albumsResponse
    ...
    <el-container>
      ...
      <el-main>
        <div style="width: auto; height: auto;">
          <el-row>
            <el-col v-for="(alaum, index) in albums" :key="alaum.id" :span="4" :offset="index > 0 ? 2 : 1">
              <el-card :body-style="{ padding: '0px' }" class="elcard">
                <el-image :src="album.forePage" class="image" :fit="fit" :preview-src-list="[image.img]" />
          ...
          <!-- <el-card :body-style="{ padding: '0px' }" class="elcard">
								<el-image src="@/assets/res1.JPG" class="image" :fit="fit" :preview-src-list="[image.img]" />
								<div style="padding: 5px">
									<el-text tag="b" size="large">{{ image.title }}</el-text>
									<div>
										<el-text class="mx-1" type="info">相册：[{{ image.alaum }}]</el-text>
